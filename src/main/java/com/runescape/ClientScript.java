...
public class ClientScript extends Class298_Sub37 {
	...
	public LinkedList[] aClass437Array9685;
	...
	int method3473(RsByteBuffer class298_sub53, int i) {
		try {
			...
			if (i_3_ > 0) {
				aClass437Array9685 = new LinkedList[i_3_];
				for (int i_4_ = 0; i_4_ < i_3_; i_4_++) {
					int i_5_ = class298_sub53.readUnsignedShort();
					LinkedList class437 = new LinkedList(Class416.method5590(i_5_, (byte) 16));
		...
		try {
			...
			if (class394 == Class394.aClass394_4925) {
				...
				if (string.toLowerCase().contains("runescape")) {
					string = string.replace("runescape", ClientSettings.SERVER_NAME);
					string = string.replace("RuneScape", ClientSettings.SERVER_NAME);
					string = string.replace("Runescape", ClientSettings.SERVER_NAME);
				}
