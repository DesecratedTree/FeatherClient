...
public class Class319_Sub1 extends Class319 implements MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public synchronized void mouseWheelMoved(MouseWheelEvent mousewheelevent) {
		try {
			int mouseWheelX = mousewheelevent.getX();
			int mouseWheelY = mousewheelevent.getY();
			int wheelRotation = mousewheelevent.getWheelRotation();

			if (mousewheelevent.isShiftDown()){
				if (ClientSettings.zoom <= 400 && wheelRotation == -1 || ClientSettings.zoom >= 1400 && wheelRotation == 1) {
					return;
				}
				int diff = wheelRotation == -1 ? -30 : 30;
				ClientSettings.zoom = ClientSettings.zoom + diff;
				return;
			}

			method3908(6, mouseWheelX, mouseWheelY, wheelRotation, 1166829560);
